...
use std::{sync::{atomic::{AtomicU16, Ordering, fence, AtomicU64, AtomicU32, AtomicU8, AtomicBool, }, mpsc::{Receiver, channel, Sender}}, mem::{size_of, MaybeUninit, forget,  transmute, align_of, transmute_copy, needs_drop, replace, ManuallyDrop}, ptr::{addr_of, null_mut, copy_nonoverlapping, addr_of_mut }, thread::{JoinHandle, self, park, Thread, sleep, spawn, current}, cell::UnsafeCell, alloc::{alloc, Layout}, marker::PhantomData, time::Duration,  collections::HashMap, os::fd::RawFd};
...
impl SubRegionAllocator {
  ...
  fn alloc_task_frame(
    ...
    frame_ref.write(TaskFrame {
      ...
      secondary_data: SecondaryData { uninit: () }
    });
...
impl TaskContext {
  ...
  pub fn access_frame_as_init<T>(&self) -> &mut T { unsafe {
    return &mut *self.acccess_frame_as_raw().cast::<T>()
  }; }
...
impl Action {
  ...
  fn project_raw_ptr(&self) -> *mut () {
    let mut ptr = 0u64;
    let src = addr_of!(unsafe { &*self.0.get() }.0) as *mut u8;
    unsafe { copy_nonoverlapping(src, addr_of_mut!(ptr) as *mut u8, 6) };
    return ptr as _;
  }
  fn new(fun_ptr: Thunk) -> Self { unsafe {
    let mut int = ActionInternals(garbage!(), TaskFlags::new());
    copy_nonoverlapping(addr_of!(fun_ptr) as *const u8, addr_of_mut!(int.0) as *mut u8, 6);
    int.1.set_thunk_type(ThunkType::Then);
    return Self(UnsafeCell::new(int))
  } }
  ...
  fn store_continuation(&mut self, next: Continuation) {
    match next.continuation {
      ...
      ContinuationRepr::AwaitIO(fd, r, w, cont) => {
        ...
        frame.secondary_data = SecondaryData { io: ManuallyDrop::new((fd, r, w)) };
      },
      ContinuationRepr::Sync(obj, asked_mut, thunk) => {
        ...
        let frame =  self.task_frame_ptr.deref_mut();
        frame.secondary_data = SecondaryData { sync: ManuallyDrop::new((obj, asked_mut)) };
      },
    }
  }
